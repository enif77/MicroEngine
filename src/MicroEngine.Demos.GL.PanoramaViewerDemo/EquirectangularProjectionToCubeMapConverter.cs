...
public class EquirectangularProjectionToCubeMapConverter
{
    ...
    public Image RenderFace(Image sourceImage, CubeMapFaceOrientation cubeMapFaceOrientation, double rotation, int maxWidth = int.MaxValue)
    {
        ...
        var imageInterpolator = new PixelInterpolator();
        ...
        for (var top = 0; top < faceSize; top += subFaceSize)
        {
            for (var left = 0; left < faceSize; left += subFaceSize)
            {
                var subFaceState = new RenderState()
                {
                    Left = left,
                    Top = top,
                    Image = new Image(subFaceSize, subFaceSize)
                };
    ...
    private static void BlitImage(RenderState source, Image destination)
    {
        ...
        for (var y = top; y < bottom; y++)
        {
            for (var x = left; x < right; x++)
            {
                for (var i = 0; i < 4; i++)
                {
                    destination.Pixels[to++] = source.Image.Pixels[from++];
                }
            }
    ...
    private static void RenderSubFace(
        ...
        double rotation,
        PixelInterpolator imageInterpolator,
        int faceSize,
        RenderState state)
    {
