...
public class PhysicsObject : IPhysicsObject
{
    ...
    /// <param name="forceOrientation">The normalized force vector to apply, typically representing thrust. It is in the local space of the physics object.</param>
    /// <param name="forceStrength">A factor to scale the force.</param>
    /// <exception cref="ArgumentException">Thrown when the force strength is negative.</exception>
    ...
    /// <summary>
    /// The force accumulator is used to accumulate forces that will be applied later.
    /// </summary>
    private Vector3 _forceAccumulator = Vector3.Zero;
    
    /// <summary>
    /// The torque accumulator is used to accumulate torques that will be applied later.
    /// </summary>
    ...
    /// </summary>
    public void ClearForceAndTorqueAccumulators()
    {
    ...
    /// <summary>
    /// Accumulates a local force to be applied later.
    /// Can be used to simulate a thruster connected to the physics object.
    /// </summary>
    /// <param name="forcePosition">The position where the force is applied, typically the center of mass or a specific point on the object.</param>
    /// <param name="forceOrientation">The normalized force vector to apply, typically representing thrust. It is in the local space of the physics object.</param>
    /// <param name="forceStrength">A factor to scale the force.</param>
    /// <exception cref="ArgumentException">Thrown when the force strength is negative.</exception>
    public void AccumulateLocalForce(Vector3 forcePosition, Vector3 forceOrientation, float forceStrength)
    {
        if (forceStrength < 0)
        {
            throw new ArgumentException("Force strength must be non-negative.", nameof(forceStrength));
        }
        ...
        // Accumulate the force in the direction of this object's orientation, scaled by the thrust factor.
        _forceAccumulator += orientedForce;
        ...
        // Accumulate the torque to the object.
        _torqueAccumulator += Vector3.Cross(r, orientedForce);
    }
    
    /// <summary>
    /// Applies all accumulated forces and torques to the object.
    /// Clears the accumulators after applying.
    /// </summary>
